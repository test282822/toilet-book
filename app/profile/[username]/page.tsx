...
import { Toilet, Calendar, Star, Coins, Trophy, Users, Accessibility } from "lucide-react"
...
export default async function ProfilePage({ params }: { params: { username: string } }) {
  ...
  const [
    ...
    supabase.from("posts").select("*", { count: "exact", head: true }).eq("user_id", profile.id).eq("has_adult_changing_station", true),
  ])

  const { data: avgData } = await supabase
    .from("posts").select("rating").eq("user_id", profile.id)

  const avgRating = avgData?.length
    ? (avgData.reduce((s, p) => s + (p.rating ?? 0), 0) / avgData.length).toFixed(1)
    : null
      ...
      <div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6">
        ...
        <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 mb-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap mb-1">
                ...
                <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${badge.bg} ${badge.color}`}>
                  {badge.emoji} {badge.label}
                </span>
              </div>
              <p className="text-sm text-slate-500 dark:text-slate-400 mb-1">@{profile.username}</p>
              {profile.bio && <p className="text-sm text-slate-600 dark:text-slate-400 mb-2">{profile.bio}</p>}
              <div className="flex items-center gap-4 text-xs text-slate-400 flex-wrap">
                <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />Joined {joinDate}</span>
                {profile.website && (
                  <a href={profile.website} target="_blank" rel="noopener noreferrer" className="text-sky-500 hover:text-sky-400">
                    {profile.website.replace(/^https?:\/\//, "")}
                  </a>
                )}
              </div>
              {badge.next && <p className="text-xs text-slate-400 mt-1.5">Next: {badge.next}</p>}
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-5 gap-3 mt-5 pt-5 border-t border-slate-100 dark:border-slate-800">
            {[
              ...
              { icon: <Coins className="h-3.5 w-3.5 text-sky-400" />,          label: "FLUSH pts",      value: fmt(flushBalance) },
              { icon: <Trophy className="h-3.5 w-3.5 text-violet-400" />,      label: "Tokens",         value: fmt(flushBalance * 10) },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="rounded-2xl bg-gradient-to-br from-sky-500/10 to-indigo-500/10 border border-sky-200 dark:border-sky-800 p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-sm font-semibold text-slate-800 dark:text-white">Your FLUSH balance</p>
              </div>
              <p className="text-3xl font-bold text-slate-900 dark:text-white mb-1">
                {fmt(flushBalance)} <span className="text-sm text-slate-400 font-normal">points</span>
              </p>
              <p className="text-sm text-sky-600 dark:text-sky-400 font-semibold mb-3">= {fmt(flushBalance * 10)} FLUSH tokens at launch</p>
              <Link href="/points" className="text-xs text-sky-500 hover:text-sky-400">See token details →</Link>
            </div>
            <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5">
              ...
              <p className="text-xs text-slate-400 mb-3">
                Earn <strong className="text-emerald-500">+50 FLUSH points</strong> for every friend who signs up
              </p>
              <div className="flex items-center gap-2 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-3 py-2 mb-2">
                <code className="text-xs text-slate-600 dark:text-slate-300 flex-1 truncate">{referralUrl}</code>
        ...
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-slate-800 dark:text-slate-200 flex items-center gap-2">
            ...
            Reviews
            <span className="text-xs text-slate-400 font-normal">({fmt(totalPosts ?? 0)})</span>
          </h2>
          {avgRating && (
            <span className="text-xs text-slate-400">Avg: <span className="text-amber-400 font-semibold">★ {avgRating}</span></span>
          )}
        </div>
        ...
        {!posts?.length ? (
          <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 py-16 text-center">
            ...
            {isOwner && <p className="text-xs text-slate-400 mt-2">Start reviewing bathrooms to earn FLUSH tokens!</p>}
          </div>
        ) : (
